...
import type { Player, WordHistoryEntry } from '../types/multiplayer';
...
export default function MultiplayerGameOverScreen({
  ...
  const myScore = playerNumber !== null ? (scores[playerNumber] ?? 0) : 0;
  const opponentScore = opponent ? (scores[opponent.number] ?? 0) : 0;
  const myWordsFound = playerNumber !== null ? (wordsFound[playerNumber] ?? 0) : 0;
  const opponentWordsFound = opponent ? (wordsFound[opponent.number] ?? 0) : 0;

  const getWordHistoryVisuals = (entry: WordHistoryEntry) => {
